...
class Helper:
    ...
    def detect_service(port: int) -> ServiceInfo:
        ...
        if info.valid and port in Helper.BLUEOS_PORTS:
    ...
    async def check_website(site: Website) -> WebsiteStatus:
        async with httpx.AsyncClient(verify=False) as client:
            try:
                response = await client.get(site.value, timeout=10)
...
if __name__ == "__main__":
    ...
    config = Config(app=app, loop=loop, host="0.0.0.0", port=9084, log_config=None)
